[assembly: System.CLSCompliantAttribute(true)]
[assembly: System.Runtime.CompilerServices.InternalsVisibleToAttribute(@"NServiceBus.CastleWindsor.Tests, PublicKey=")]
[assembly: System.Runtime.InteropServices.ComVisibleAttribute(false)]
[assembly: System.Runtime.Versioning.TargetFrameworkAttribute(".NETFramework,Version=v4.5.2", FrameworkDisplayName=".NET Framework 4.5.2")]

namespace NServiceBus
{
    
    public class WindsorBuilder : NServiceBus.Container.ContainerDefinition
    {
        public WindsorBuilder() { }
        public override NServiceBus.ObjectBuilder.Common.IContainer CreateContainer(NServiceBus.Settings.ReadOnlySettings settings) { }
    }
    public class static WindsorExtensions
    {
        public static void ExistingContainer(this NServiceBus.Container.ContainerCustomizations customizations, Castle.Windsor.IWindsorContainer container) { }
    }
}